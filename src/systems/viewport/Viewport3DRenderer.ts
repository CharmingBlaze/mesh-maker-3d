...
import { boundsSize, boundsCenter } from '@/core/math/BoundingBox';
...
export class Viewport3DRenderer {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    const rimLight = new THREE.DirectionalLight(0xa8ff3e, 0.15);
  ...
  setCadPrimPreview(draw: PrimDrawState | null): void {
  ...
  rebuild(
    ...
    if (selEdges.size > 0) {
      ...
      if (points.length > 0) {
        ...
        this.meshGroup.add(
          new THREE.LineSegments(
            geo,
            new THREE.LineBasicMaterial({ color: 0xf8c156, linewidth: 2 }),
          ),
        );
      }
    }
    ...
    addVertexInstances(normalVerts, 3, 0x78a8d8);
    addVertexInstances(selectedVerts, 5, 0xf3c56a);
    this.needsRender = true;
  }
