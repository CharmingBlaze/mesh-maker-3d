...
export function LayersPanel() {
  ...
  const resolveDropIndex = useCallback((clientY: number): number | null => {
    ...
    if (!list) return null;
    const rows = list.querySelectorAll<HTMLElement>('[data-layer-index]');
  ...
  return (
    <div className="ms-layers-tab">
      <div className="ms-layers-toolbar">
        <button type="button" className="ms-btn ms-layer-tb-btn" onClick={addLayer} title="Add layer">
          + Add
        </button>
        <button
          type="button"
        ...
        <button type="button" className="ms-btn ms-layer-tb-btn" onClick={assignSelectionToLayer} title="Assign selection to active layer">
          Assign
        </button>
      </div>
      ...
      <div className="ms-layer-list" ref={listRef} role="list">
        {layers.length === 0 ? (
          <div className="ms-empty">No layers — click Add</div>
        ) : (
          layers.map((layer, index) => (
            ...
            >
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  if (layers.length > 1 && confirm(`Delete layer "${layer.name}"?`)) {
                    deleteLayer(index);
                  }
                }}
              >
      ...
      {active && (
        <div className="ms-layers-footer">
          ...
          <span className="ms-layers-hint">Double-click name to rename · drag ⋮⋮ to reorder</span>
        </div>
      )}
    </div>
  );
